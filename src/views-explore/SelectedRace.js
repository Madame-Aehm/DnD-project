...
function SelectedRace() {
  ...
  async function addFavourite() {
    if (alreadyFavourited) {
      alert(selectedRace.name + " is already in your favourites.")
    } else {
      try {
        ...
        alert(selectedRace.name + " added to Favourites!")
        getLocalFavourites();
      } catch (e) {
  ...
  function checkForFav() {
    ...
    for (let i = 0; i < favouritesArray.length; i++) {
      if (favouritesArray[i].name === selectedRace.name) {
    ...
    <div className='content-container'>
      ...
      {!pageLoader && 
        <>
            ...
            <div className='add-fav-container'>
              ...
              {user && 
                <>
                  ...
                  {alreadyFavourited && <button className='fav-button' title={selectedRace.name + " is in your Favourites"} disabled>♥</button>}
                </>
              }
            </div>
          ...
          {selectedRace.race && <DisplaySubrace props={selectedRace} handleOnClick={handleOnClick} />}
        </>
      }
    </div>
  )
