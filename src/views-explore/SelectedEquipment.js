...
function SelectedEquipment() {
  ...
  async function addFavourite() {
    if (alreadyFavourited) {
      alert(selectedEquipment.name + " is already in your favourites.")
    } else {
      try {
        const docRef = await addDoc(collection(db, "Favourites_user" + user.uid), {
          name: selectedEquipment.name,
          url: selectedEquipment.url,
          index: selectedEquipment.index,
          fav_category: category
        });
        console.log("Document written with ID: ", docRef.id);
        alert(selectedEquipment.name + " added to Favourites!")
        getLocalFavourites();
      } catch (e) {
  ...
  function checkForFav() {
    ...
    for (let i = 0; i < favouritesArray.length; i++) {
      if (favouritesArray[i].name === selectedEquipment.name) {
                ...
                <div className='add-fav-container'>
                  ...
                  {user && 
                    <>
                      ...
                      {alreadyFavourited && <button className='fav-button' title={selectedEquipment.name + " is in your Favourites"} disabled>♥</button>}
                    </>
                  }
                </div>
