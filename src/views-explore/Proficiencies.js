...
function Proficiencies() {
  ...
  const {
    ...
    pageLoader,
    error,
  } = useMainFetch("https://www.dnd5eapi.co/api/proficiencies");
          ...
          <>
            ...
            <div className='explore-list'>
              {filteredList.map((item) => {
                return (
                  <Link className='explore-button' to={"/selectedproficiency"} state={{url: item.url, array: filteredList, searchResult: filter}} key={item.index}>{item.name}</Link>
                )
              })}
